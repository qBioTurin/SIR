...
SensitivityPlot <-function(rank=T,folder){
  load(paste0(folder,"SIR_age-sensitivity.RData"))
  ...
  if(rank){
    load(paste0(folder,"ranking_SIR_age-sensitivity.RData"))
  }else{
    rank <- data.frame(measure = 0 , id = paste0("SIR_age-sensitivity-",
                                                 id.traces,
                                                 ".trace"))
  }
  ...
  ListTraces<-lapply(id.traces,
                     function(x){
                       trace.tmp=read.csv(paste0(
                        folder,"/SIR_age-sensitivity-",
                         x,
                         ".trace"), sep = "")
                       trace.tmp=data.frame(trace.tmp,ID= x,
                                            rank = rank[which(rank[,2]==paste0("SIR_age-sensitivity-",
                                                                               x,
                                                                               ".trace")),1])
                       return(trace.tmp)
                     })
  ...
  return(list(TrajS_a1 =plS_a1, TrajS_a2 =plS_a2,TrajI_a1 =plI_a1,TrajI_a2 =plI_a2,TrajR_a1 =plR_a1,TrajR_a2 =plR_a2))
}
