...
ModelAnalysisPlot=function(solverName_path,Stoch = F,print=T){
  ...
  colors <- c("Reference data" = "red", "Output data" = "grey64")

  plI<-ggplot( )+
    geom_line(data=trace,
              aes(x=Time,y=I,group=ID, color="Output data"))+
    ...
    labs(x="Days", y="I", color="Type of data")
  ...
  if(Stoch){
    ...
    plS<-plS+
      geom_line(data=meanTrace,
                aes(x=Time,y=S,color="Mean output data"),
                linetype="dashed")+
      labs(x="Days", y="S")

    plR<-plR+
      geom_line(data=meanTrace,
                aes(x=Time,y=R,color="Mean output data"),
                linetype="dashed")+
      labs(x="Days", y="R")
